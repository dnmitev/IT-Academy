(function() {
    ...
    function getWordCountInText(text, word) {
        var count = 0;
        var start = 0;
        
        while (text.indexOf(word, start) >= 0) {
            let currentIndex = text.indexOf(word, start);
            
            count++;
            start = currentIndex + word.length; // eq. start = start + word.length
        }

        return count;
    }

    var count = getWordCountInText(loremIpsum, word);
    console.log(count);
}());